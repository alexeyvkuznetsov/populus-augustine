...
| `figures/` | Рисунки, которые создают ноутбуки: рис. 1–5 и П1–П2 статьи, отдельные сообщества (`communities/`), локальные сети всех 18 целевых лемм (`local_networks/`), матрица сходства целевых лемм |
...
| `Fig2_local_networks_populus_gens.png` | Рис. 2. Локальные сети *populus* (A) и *gens* (B) |
| `Fig3_local_network_plebs.png` | Рис. 3. Локальная сеть *plebs* |
| `Fig4_local_network_civitas.png` | Рис. 4. Локальная сеть *civitas* |
| `Fig5_umap_leiden_hdbscan.png` | Рис. 5. Сообщества Лейдена и кластеры HDBSCAN |
...
Все случайные процедуры используют фиксированные начальные значения. Результаты основного анализа (порог, сеть, сообщества, соседи, локальные сети) полностью воспроизводимы. Результат UMAP зависит также от версии библиотек и платформы, поэтому число кластеров HDBSCAN на рис. 5 при повторном запуске может отличаться на несколько единиц; выводы статьи опираются на многократную проверку по 120 проекциям, которая от этого не зависит. Раскладка узлов на рисунках сетей зависит от версии `networkx`.
